...
import { Locators } from "../vscode-project-manager-core/src/locator/locators";
...
import { Suggestion } from "../vscode-project-manager-core/src/Suggestion";
...
export function activate(context: vscode.ExtensionContext) {
    ...
    function showListProjectsQuickPick(folderNotFound: (name: string, path: string) => void): Promise<Project | undefined> {
        let items = [];
        const filterByTags = Container.context.globalState.get<string[]>("filterByTags", []);
        items = projectStorage.getProjectsByTags(filterByTags);//map();
